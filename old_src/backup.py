...
str1 = 'after that alice said the king and'
...
# Assuming you have the following predictions from your trigram model
trigram_predictions = pred_1[1]

# Generate sentences using the predicted words
generated_sentences = []
for _ in range(5):  # Adjust the number of sentences you want to generate
    sentence = ['after', 'that', 'alice', 'said', 'the']  # Starting seed words
    for _ in range(10):  # Adjust the number of words you want in each sentence
        next_word = random.choice(trigram_predictions)
        sentence.append(next_word)
        if next_word == 'NOT FOUND':
            break  # Stop if 'NOT FOUND' is encountered
